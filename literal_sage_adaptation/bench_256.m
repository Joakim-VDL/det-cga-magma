...
load "literal_sage_adaptation/twisted_Orient.m";
load "literal_sage_adaptation/data_256.m";

// 1. Initial setup
Fp := GF(p);
// Since p = 3 mod 4, X^2 + 1 is irreducible
...
if #sigma eq 4 then
    ...
    alpha := Fp2 ! [Fp!sigma[1], Fp!sigma[2]];
else
...
for entry in base_cycle do
    ...
    xPt_val := Fp2 ! [entry[3][1], entry[3][2]];
    xQs_val := Fp2 ! [entry[4][1], entry[4][2]];
    xQt_val := Fp2 ! [entry[5][1], entry[5][2]];
    Append(~aff_base_cycle, [* A_val, xPs_val, xPt_val, xQs_val, xQt_val *]);
end for;
...
fact_t := Factorization(Mt);
params`ells_twist := [f[1] : f in fact_t];
params`exps_twist := [f[2] : f in fact_t];

params`base_cycle := aff_base_cycle;

function exps_to_B(exps_s, exps_t, lmbda)
    B := 0;
    while true do
        B +:= 1;
        key_space_size := &+([0.0] cat [Log(2, B * e + 1.0) : e in exps_s]) + &+([0.0] cat [Log(2, B * e + 1.0) : e in exps_t]);
...
lmbda := 256;
B := exps_to_B(params`exps_straight, params`exps_twist, lmbda);
...
for i := 1 to num_iters do
    ...
    keygen(~sk_alice, B, params`exps_straight, params`exps_twist);
    keygen(~sk_bob, B, params`exps_straight, params`exps_twist);
...
med := Median(timings);
printf "Timing results for a 256-bit group action (literal adaptation)\n";
